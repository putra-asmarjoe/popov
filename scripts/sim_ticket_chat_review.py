"""Simulasi percakapan chat tiket proxmox — review relevansi jawaban & chip.
...
SCENARIOS = {
    "PROX-1": [  # open critical etcdInsufficientMembers
        "What is this ticket about?",
        "How many times has this etcd alert fired?",
        "Can you summarize the alerts linked to this ticket?",
        "Is this related to a database problem or a kubernetes issue?",
        "Who can I assign this ticket to?",
        "What should be checked first to fix this?",
        "Add a progress note saying we are checking etcd member count",
        "What is the current status of this ticket?",
    ],
    "PROX-3": [  # resolved high TargetDown scheduler
        "Summarize this ticket",
        "What was the root cause here?",
        "When was this ticket resolved?",
        "Is the scheduler pod healthy now?",
        "Reopen this ticket please",
        "What is the current status?",
        "Show me the progress log entries",
        "What alerts are linked to this ticket?",
    ],
    "PROX-6": [  # new high KubeSchedulerInstanceUnreachable
        "Ticket detail",
        "Why is the scheduler instance unreachable?",
        "What severity is this ticket and why?",
        "Can you check if there are similar incidents for this service?",
        "What does KubeSchedulerInstanceUnreachable mean?",
        "Assign this ticket to Iva",
...
async def main():
    db = get_db()
    proj = await db["projects"].find_one({"slug": "proxmox"})
    pid = str(proj["_id"])
    report = {}
